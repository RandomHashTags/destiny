...
let router:Router = #router(
    ...
    middleware: [
        ...
        StaticMiddleware(
            handlesMethods: [.get],
            handlesStatuses: [.notImplemented],
            handlesContentTypes: [
                HTTPMediaType.textHtml,
                HTTPMediaType.applicationJson,
                HTTPMediaType.textPlain
            ],
            appliesStatus: .ok
        ),
        ...
        DynamicMiddleware({ request, response in
            ...
            response.message.headers["Womp-Womp"] = UUID().uuidString
            #else
            response.message.headers["Womp-Womp"] = String(UInt64.random(in: 0..<UInt64.max))
            #endif
        })
    ],
    redirects: [
        ...
        RouterGroup(
            ...
            DynamicRoute(
                ...
                handler: { _, response in
                    response.message.result = .string("RLY DUD")
                }
            )
        ),
    ],
    ...
    StaticRoute.get(
        ...
        caseSensitive: false,
        contentType: HTTPMediaType.textHtml,
        result: .staticString(#"<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body><h1>shooped</h1></body></html>"#)
    ),
    StaticRoute.get(
        version: .v2_0,
        path: ["html2"],
        contentType: HTTPMediaType.textHtml,
        result: .staticString(#"<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body><h1>you did html2</h1></body></html>"#)
    ),
    StaticRoute.get(
        path: ["json"],
        contentType: HTTPMediaType.applicationJson,
    ...
    DynamicRoute.get(
        ...
        contentType: HTTPMediaType.textPlain,
        handler: { request, response in
            response.message.result = .string("bro")
    ...
    DynamicRoute.get(
        ...
        handler: { request, response in
            #if canImport(FoundationEssentials) || canImport(Foundation)
            response.message.result = .string(UUID().uuidString)
            #else
    ...
    DynamicRoute.get(
        ...
        contentType: HTTPMediaType.textPlain,
        handler: { request, response in
            response.message.result = .string(response.parameters[0])
        }
    ),
    DynamicRoute.get(
    ...
    DynamicRoute.get(
        ...
        contentType: HTTPMediaType.textPlain,
        handler: { request, response in
            response.message.result = .string(response.parameters.description)
        }
    )
)
let server:Server = try Server(
    port: 8080,
    router: router,
