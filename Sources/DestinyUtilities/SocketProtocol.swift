...
extension SocketProtocol where Self: ~Copyable {
    ...
    public func readBytes(length: Int) throws -> [UInt8] {
        return try [UInt8](
            ...
            initializingWith: { $1 = try readBuffer(into: $0, length: length) })
    }
    ...
    @inlinable
    public func readHeaders() throws -> [String: String] {
        var headers: [String: String] = [:]
        while case let line:String = try readLine(), !line.isEmpty {
            let values: [Substring] = line.split(
    ...
    @inlinable
    public func readBuffer(into buffer: UnsafeMutableBufferPointer<UInt8>, length: Int) throws
        -> Int
    {
    ...
    @inlinable
    public func readBufferIfNotCancelled(
        into buffer: UnsafeMutableBufferPointer<UInt8>, length: Int
    ) throws
        -> Int
    {
        var bytes_read: Int = 0
        guard let baseAddress: UnsafeMutablePointer<UInt8> = buffer.baseAddress else { return 0 }
        while bytes_read < length {
            if Task.isCancelled { throw SocketError.operationCancelled }
            let to_read: Int = min(Self.bufferLength, length - bytes_read)
            let read_bytes: Int = read(fileDescriptor, baseAddress + bytes_read, to_read)
            if read_bytes < 0 {  // error
                throw SocketError.readBufferFailed()
...
extension SocketProtocol where Self: ~Copyable {
    ...
    public func writeBuffer(_ pointer: UnsafeRawPointer, length: Int) throws {
        ...
        while sent < length {
            #if os(Linux)
                let result: Int = send(
                    fileDescriptor, pointer + sent, length - sent, Int32(MSG_NOSIGNAL))
            #else
    ...
    @inlinable
    public func writeBufferIfNotCancelled(_ pointer: UnsafeRawPointer, length: Int) throws {
        guard !closed else { return }
        var sent: Int = 0
        while sent < length {
            if Task.isCancelled { throw SocketError.operationCancelled }
