    ...
    package func response(version: String, middleware: [any MiddlewareProtocol]) -> String {
        ...
        for middleware in middleware {
            if middleware.appliesToMethods.contains(method)
                && middleware.appliesToContentTypes.contains(contentType)
                && (response_status != nil
                    ? middleware.appliesToStatuses.contains(response_status!) : true)
            {
        ...
        let result: Result = staticResult ?? dynamicResult!(nil)
        let result_string: String =
            ...
            case .bytes(let bytes):
                bytes.withUnsafeBufferPointer { buffer in
                    escapeString(for: contentType, buffer: buffer) ?? ""
                }
            }
        var string: String =
            version + " \(response_status ?? HTTPResponse.Status.notImplemented)\\r\\n"
...
func escapeString(for contentType: Route.ContentType, buffer: UnsafeBufferPointer<UInt8>) -> String?
{
    switch contentType {
    case .text:
        return String(data: Data(buffer), encoding: .utf8)
    case .html:
        return String(data: Data([UInt8(ascii: " ")] + buffer), encoding: .utf8)
    case .json:
        return String(data: Data(buffer), encoding: .utf8)?.replacing(
            "\"", with: #"\""#)
    }
}
