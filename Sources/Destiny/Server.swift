...
public final class Server: Service {
    ...
    public func run() async throws {
        ...
        binded = withUnsafePointer(to: &addr) {
            bind(
                serverFD, UnsafePointer<sockaddr>(OpaquePointer($0)),
        ...
        let not_found_response: StaticString = StaticString(
            "HTTP/1.1 200 OK\r\nContent-Type: text/plain; charset=utf-8\r\nContent-Length:9\r\n\r\nnot found"
        )
        logger.notice(
            Logger.Message(
                stringLiteral:
                    "Listening for clients on http://\(address ?? "localhost"):\(port) [maxPendingConnections=\(maxPendingConnections)]"
            ))
                    ...
                    for _ in 0..<maxPendingConnections {
                        group.addTask {
                            do {
                                ...
                                try await Self.process_client(
                                    client: client, static_responses: static_responses,
    ...
    static func client(fileDescriptor: Int32) throws -> Int32 {
        ...
        return try withUnsafeMutablePointer(to: &addr) { addrPtr in
            let client: Int32 = accept(
                fileDescriptor,
                UnsafeMutableRawPointer(addrPtr).assumingMemoryBound(to: sockaddr.self),
                &len)
            if client == -1 {
