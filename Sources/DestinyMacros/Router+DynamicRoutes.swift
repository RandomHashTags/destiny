...
extension Router.Storage {
    ...
    mutating func getResponderValue(
        ...
        if let responderBody = logicSplit.getPositive(1), let parameters = responderBody.firstIndex(of: "\n") {
            ...
            let name = "DynamicResponder\(autoGeneratedResponderIndex)"
            let autoGenStruct = """
            // MARK: \(name)
            struct \(name)<let pathCount: Int>: DynamicRouteResponderProtocol {
            ...
            responder = "DynamicResponder\(autoGeneratedResponderIndex)(\(initializer))"
